    ...
    public class ExcelGenerateService
    {
        ...
        public string ExcelCDDIS(CDDISValuesVIewModel data, string newFileName)
        {
            ...
            using (ExcelPackage package = new ExcelPackage(newFile, templateFile))
            {
                ...
                ws.Cells["E14"].Value = data.DEBIT_CREDIT_1;
                ws.Cells["E25"].Value = data.DEBIT_CREDIT_2;

                //CCY 1 & 2
                colpnt = 12;
                strLength = (data.CCY_1.Length <= 4) ? data.CCY_1.Length : 4;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "14"].Value = data.CCY_1.Substring(c, 1);
                    colpnt++;
                }
                colpnt = 12;
                strLength = (data.CCY_2.Length <= 4) ? data.CCY_2.Length : 4;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "25"].Value = data.CCY_2.Substring(c, 1);
                    colpnt++;
                }

                //AMOUNT 1 & 2
                string amount_1 = String.Format("{0:#,##0.##}", data.AMOUNT_1);
                string amount_2 = String.Format("{0:#,##0.##}", data.AMOUNT_2);
                ...
                //CUSTOMER ABBR 1 & 2
                colpnt = 4;
                strLength = (data.CUSTOMER_ABBR_1.Length <= 12) ? data.CUSTOMER_ABBR_1.Length : 12;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "15"].Value = data.CUSTOMER_ABBR_1.Substring(c, 1);
                    colpnt++;
                }
                colpnt = 4;
                strLength = (data.CUSTOMER_ABBR_2.Length <= 12) ? data.CUSTOMER_ABBR_2.Length : 12;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "26"].Value = data.CUSTOMER_ABBR_2.Substring(c, 1);
                    colpnt++;
                }

                //ACCOUNT CODE 1 & 2
                colpnt = 4;
                strLength = (data.ACCOUNT_CODE_1.Length <= 5) ? data.ACCOUNT_CODE_1.Length : 5;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "16"].Value = data.ACCOUNT_CODE_1.Substring(c, 1);
                    colpnt++;
                }
                colpnt = 4;
                strLength = (data.ACCOUNT_CODE_2.Length <= 5) ? data.ACCOUNT_CODE_2.Length : 5;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "27"].Value = data.ACCOUNT_CODE_2.Substring(c, 1);
                    colpnt++;
                }

                //ACCOUNT NO 1 & 2
                ws.Cells["Z16"].Value = data.ACCOUNT_NO_1.Substring(0, 1);
                ws.Cells["AA16"].Value = data.ACCOUNT_NO_1.Substring(1, 1);
                ws.Cells["AB16"].Value = data.ACCOUNT_NO_1.Substring(2, 1);
                ws.Cells["AD16"].Value = data.ACCOUNT_NO_1.Substring(3, 1);
                ws.Cells["AE16"].Value = data.ACCOUNT_NO_1.Substring(4, 1);
                ws.Cells["AF16"].Value = data.ACCOUNT_NO_1.Substring(5, 1);
                colpnt = 33;
                strLength = (data.ACCOUNT_NO_1.Length <= 12) ? data.ACCOUNT_NO_1.Length : 12;
                for (int c = 6; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "16"].Value = data.ACCOUNT_NO_1.Substring(c, 1);
                    colpnt++;
                }

                ws.Cells["Z27"].Value = data.ACCOUNT_NO_2.Substring(0, 1);
                ws.Cells["AA27"].Value = data.ACCOUNT_NO_2.Substring(1, 1);
                ws.Cells["AB27"].Value = data.ACCOUNT_NO_2.Substring(2, 1);
                ws.Cells["AD27"].Value = data.ACCOUNT_NO_2.Substring(3, 1);
                ws.Cells["AE27"].Value = data.ACCOUNT_NO_2.Substring(4, 1);
                ws.Cells["AF27"].Value = data.ACCOUNT_NO_2.Substring(5, 1);
                colpnt = 33;
                strLength = (data.ACCOUNT_NO_2.Length <= 12) ? data.ACCOUNT_NO_2.Length : 12;
                for (int c = 6; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "27"].Value = data.ACCOUNT_NO_2.Substring(c, 1);
                    colpnt++;
                }

                //EXCHANGE RATE 1 & 2
                string exrate_1 = String.Format("{0:#,##0.####}", data.EXCHANGE_RATE_1);
                string exrate_2 = String.Format("{0:#,##0.####}", data.EXCHANGE_RATE_2);
                ...
                //CONTRA CCY 1 & 2
                colpnt = 25;
                strLength = (data.CONTRA_CCY_1.Length <= 4) ? data.CONTRA_CCY_1.Length : 4;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "17"].Value = data.CONTRA_CCY_1.Substring(c, 1);
                    colpnt++;
                }

                colpnt = 25;
                strLength = (data.CONTRA_CCY_2.Length <= 4) ? data.CONTRA_CCY_2.Length : 4;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "28"].Value = data.CONTRA_CCY_2.Substring(c, 1);
                    colpnt++;
                }

                //FUND 1 & 2
                ws.Cells["E18"].Value = data.FUND_1.Substring(0, 1);
                ws.Cells["E29"].Value = data.FUND_2.Substring(0, 1);

                //CHECK NO 1 & 2
                colpnt = 11;
                strLength = (data.CHECK_NO_1.Length <= 10) ? data.CHECK_NO_1.Length : 10;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "18"].Value = data.CHECK_NO_1.Substring(c, 1);
                    colpnt++;
                }

                colpnt = 11;
                strLength = (data.CHECK_NO_2.Length <= 10) ? data.CHECK_NO_2.Length : 10;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "29"].Value = data.CHECK_NO_2.Substring(c, 1);
                    colpnt++;
                }

                //AVAILABLE(DATE) 1 & 2
                ws.Cells["AH18"].Value = data.AVAILABLE_DATE_1.Month.ToString("d2").Substring(0, 1);
                ws.Cells["AI18"].Value = data.AVAILABLE_DATE_1.Month.ToString("d2").Substring(1, 1);
                ws.Cells["AJ18"].Value = data.AVAILABLE_DATE_1.Date.ToString("dd").Substring(0, 1);
                ws.Cells["AK18"].Value = data.AVAILABLE_DATE_1.Date.ToString("dd").Substring(1, 1);
                ws.Cells["AL18"].Value = data.AVAILABLE_DATE_1.Year.ToString().Substring(2, 1);
                ws.Cells["AM18"].Value = data.AVAILABLE_DATE_1.Year.ToString().Substring(3, 1);

                ws.Cells["AH29"].Value = data.AVAILABLE_DATE_2.Month.ToString("d2").Substring(0, 1);
                ws.Cells["AI29"].Value = data.AVAILABLE_DATE_2.Month.ToString("d2").Substring(1, 1);
                ws.Cells["AJ29"].Value = data.AVAILABLE_DATE_2.Date.ToString("dd").Substring(0, 1);
                ws.Cells["AK29"].Value = data.AVAILABLE_DATE_2.Date.ToString("dd").Substring(1, 1);
                ws.Cells["AL29"].Value = data.AVAILABLE_DATE_2.Year.ToString().Substring(2, 1);
                ws.Cells["AM29"].Value = data.AVAILABLE_DATE_2.Year.ToString().Substring(3, 1);

                //ADVICE PRINT 1 & 2
                ws.Cells["E19"].Value = data.ADVICE_1.Substring(0, 1);
                ws.Cells["E30"].Value = data.ADVICE_2.Substring(0, 1);

                //DETAILS 1 & 2
                colpnt = 9;
                strLength = (data.DETAILS_1.Length <= 30) ? data.DETAILS_1.Length : 30;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "19"].Value = data.DETAILS_1.Substring(c, 1);
                    colpnt++;
                }

                colpnt = 9;
                strLength = (data.DETAILS_2.Length <= 30) ? data.DETAILS_2.Length : 30;
                for (int c = 0; c < strLength; c++)
                {
                    ws.Cells[col[colpnt] + "30"].Value = data.DETAILS_2.Substring(c, 1);
                    colpnt++;
                }

                //ENTITY 1 & 2
                ws.Cells["E20"].Value = data.ENTITY_1.Substring(0, 1);
                ws.Cells["F20"].Value = data.ENTITY_1.Substring(1, 1);
                ws.Cells["G20"].Value = data.ENTITY_1.Substring(2, 1);

                ws.Cells["E31"].Value = data.ENTITY_2.Substring(0, 1);
                ws.Cells["F31"].Value = data.ENTITY_2.Substring(1, 1);
                ws.Cells["G31"].Value = data.ENTITY_2.Substring(2, 1);

                //DIVISION 1 & 2
                ws.Cells["M20"].Value = data.DIVISION_1.Substring(0, 1);
                ws.Cells["N20"].Value = data.DIVISION_1.Substring(1, 1);

                ws.Cells["M31"].Value = data.DIVISION_2.Substring(0, 1);
                ws.Cells["N31"].Value = data.DIVISION_2.Substring(1, 1);

                //INTER AMOUNT 1 & 2
                string interamount_1 = String.Format("{0:#,##0.##}", data.INTER_AMOUNT_1);
                string interamount_2 = String.Format("{0:#,##0.##}", data.INTER_AMOUNT_2);
                ...
                //INTER RATE 1 & 2
                string interrate_1 = String.Format("{0:#,##0.####}", data.INTER_RATE_1);
                string interrate_2 = String.Format("{0:#,##0.####}", data.INTER_RATE_2);
